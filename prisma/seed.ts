...
async function main() {
  ...
  const crewMembers = await prisma.crewMember.createMany({
    data: [
      { employeeId: 'CA-P001', name: '张建国', role: CrewRole.CAPTAIN, status: CrewStatus.AVAILABLE, baseAirport: 'PEK', qualifications: JSON.stringify(['A320', 'A330', 'B737']), totalFlightHours: 12500, dateOfHire: dayjs().subtract(15, 'year').toDate(), phone: '13900139001', email: 'zhangjianguo@airchina.com' },
      { employeeId: 'CA-P002', name: '李淑芬', role: CrewRole.FIRST_OFFICER, status: CrewStatus.AVAILABLE, baseAirport: 'PEK', qualifications: JSON.stringify(['A320', 'B737']), totalFlightHours: 6800, dateOfHire: dayjs().subtract(8, 'year').toDate(), phone: '13900139002', email: 'lishufen@airchina.com' },
      { employeeId: 'CA-P003', name: '王建华', role: CrewRole.CAPTAIN, status: CrewStatus.AVAILABLE, baseAirport: 'PEK', qualifications: JSON.stringify(['B777', 'B787', 'A350']), totalFlightHours: 15200, dateOfHire: dayjs().subtract(18, 'year').toDate(), phone: '13900139003', email: 'wangjianhua@airchina.com' },
      { employeeId: 'CA-F001', name: '陈美玲', role: CrewRole.FLIGHT_ATTENDANT, status: CrewStatus.AVAILABLE, baseAirport: 'PEK', qualifications: JSON.stringify(['国际航线', '国内航线', '头等舱服务']), totalFlightHours: 4500, dateOfHire: dayjs().subtract(6, 'year').toDate(), phone: '13900139004', email: 'chenmeiling@airchina.com' },
      { employeeId: 'CA-F002', name: '刘伟', role: CrewRole.FLIGHT_ATTENDANT, status: CrewStatus.AVAILABLE, baseAirport: 'PEK', qualifications: JSON.stringify(['国际航线', '国内航线']), totalFlightHours: 3200, dateOfHire: dayjs().subtract(4, 'year').toDate(), phone: '13900139005', email: 'liuwei@airchina.com' },
      { employeeId: 'CA-F003', name: '赵丽娜', role: CrewRole.PURSER, status: CrewStatus.AVAILABLE, baseAirport: 'PEK', qualifications: JSON.stringify(['国际航线', '国内航线', '乘务长资质']), totalFlightHours: 8900, dateOfHire: dayjs().subtract(10, 'year').toDate(), phone: '13900139006', email: 'zhaolina@airchina.com' },
      { employeeId: 'CA-P004', name: '孙明', role: CrewRole.FIRST_OFFICER, status: CrewStatus.AVAILABLE, baseAirport: 'PEK', qualifications: JSON.stringify(['A330', 'B777']), totalFlightHours: 5600, dateOfHire: dayjs().subtract(7, 'year').toDate(), phone: '13900139007', email: 'sunming@airchina.com' },
      { employeeId: 'CA-F004', name: '周小红', role: CrewRole.FLIGHT_ATTENDANT, status: CrewStatus.AVAILABLE, baseAirport: 'PEK', qualifications: JSON.stringify(['国内航线']), totalFlightHours: 1800, dateOfHire: dayjs().subtract(2, 'year').toDate(), phone: '13900139008', email: 'zhouxiaohong@airchina.com' },
    ],
    skipDuplicates: true,
  });
